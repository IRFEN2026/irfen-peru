#!/usr/bin/env python3
"""Freeze exact official ANA Cuenca Chicama geometry for discovery/context only."""
...
DISCOVERY_ID="lalibertad_chicama"
...
SOURCE=ROOT/"site/data/phase2/sources/north_coast_discovery_geometry/ana_lalibertad_chicama_13772.geojson"
GEOMETRY=ROOT/"site/data/phase2/geometries/lalibertad_chicama_basin_context.geojson"
VALIDATION=ROOT/"site/data/phase2/geometries/lalibertad_chicama_geometry_validation.json"
...
def validate_inputs(c,p):
 ...
 if code!="13772" or name!="Cuenca Chicama": raise ProbeError("UNEXPECTED_CHICAMA_IDENTITY")
 ...
 must=set(ci.get("must_not_merge_with") or []) | set(pi.get("must_not_merge_with") or [])
 if not {"Cuenca Jequetepeque","Cuenca Chaman","Cuenca Moche"}.issubset(must): raise ProbeError("BASIN_SEPARATION_GUARD_MISSING")
 if not ci.get("named_ravines_require_independent_geometry") or not pi.get("named_ravines_requiring_independent_geometry"): raise ProbeError("NAMED_RAVINE_SEPARATION_GUARD_MISSING")
 ...
 if q.get("endpoint")!=APPROVED_ENDPOINT or q.get("where")!="CODIGO='13772'": raise ProbeError("QUERY_NOT_LOCKED_TO_EXACT_ANA_UNIT")
...
def normalized(f,code,name,source_hash):
 props={"unit_id":DISCOVERY_ID,"name":name,"official_unit_code":code,"source_id":"ANA-UH-13772-CHICAMA","source_snapshot_sha256":source_hash,"representation":"OFFICIAL_ANA_HYDROGRAPHIC_UNIT_CONTEXT","context_only":True,"counts_as_event_footprint":False,"counts_as_operational_geometry":False,**SAFE,"alerting_enabled":False}
 return {"type":"FeatureCollection","properties":{**SAFE,"context_only":True,"source_id":"ANA-UH-13772-CHICAMA","source_snapshot_sha256":source_hash},"features":[{"type":"Feature","properties":props,"geometry":f["geometry"]}]}
