#!/usr/bin/env python3
"""Query fixed candidate-only USGS Landsat 7 STAC metadata windows; never read pixel assets."""
...
def is_l7(item):
    ...
    return iid.startswith('LE07_') or platform in ('landsat-7','landsat7')
...
def main():
    ...
    anchor=parse_utc(co['event_anchor_utc']); session=requests.Session(); session.headers.update({'User-Agent':'IRFEN-research-cleanroom/l7-availability-0.1'})
    rows=[]
    for f in sorted(feats,key=lambda z:str((z.get('properties') or {}).get('candidate_code',''))):
        ...
        for wn in ('pre','post'):
            ...
            items=[normalize(x,digest) for x in doc['features'] if is_l7(x)]
