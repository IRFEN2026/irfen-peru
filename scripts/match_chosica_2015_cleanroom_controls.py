...
FORBIDDEN_TEXT=("cashahuacra","quirio","pedregal","san_antonio","san antonio","la_libertad","la libertad","carossio","carosio","rayos_de_sol","rayos de sol","corrales","a6680","ingemmet","official_outcome_evidence","outcome_label","damage","severity","post_event","web_search")
...
def main():
    ...
    if any(x in low for x in FORBIDDEN_TEXT): raise SystemExit("FAIL_CLOSED_FORBIDDEN_TEXT_IN_CLEANROOM_INPUT")
    d=json.loads(raw)
    ...
    if any(x in text.lower() for x in FORBIDDEN_TEXT): raise SystemExit("FAIL_CLOSED_FORBIDDEN_TEXT_IN_MATCH_OUTPUT")
