#!/usr/bin/env python3
"""Freeze exact official ANA Cuenca Moche geometry for discovery/context only."""
...
SOURCE=ROOT/"site/data/phase2/sources/north_coast_discovery_geometry/ana_lalibertad_moche_137716.geojson"
...
def validate_inputs(c,p):
 ...
 if code!="137716" or name!="Cuenca Moche": raise ProbeError("UNEXPECTED_MOCHE_IDENTITY")
 ...
 if q.get("endpoint")!=APPROVED_ENDPOINT or q.get("where")!="CODIGO='137716'": raise ProbeError("QUERY_NOT_LOCKED_TO_EXACT_ANA_UNIT")
...
def normalized(f,code,name,source_hash):
 props={"unit_id":DISCOVERY_ID,"name":name,"official_unit_code":code,"source_id":"ANA-UH-137716-MOCHE","source_snapshot_sha256":source_hash,"representation":"OFFICIAL_ANA_HYDROGRAPHIC_UNIT_CONTEXT","context_only":True,"counts_as_event_footprint":False,"counts_as_operational_geometry":False,**SAFE,"alerting_enabled":False}
 return {"type":"FeatureCollection","properties":{**SAFE,"context_only":True,"source_id":"ANA-UH-137716-MOCHE","source_snapshot_sha256":source_hash},"features":[{"type":"Feature","properties":props,"geometry":f["geometry"]}]}
