#!/usr/bin/env python3
"""Freeze official ANA Chancay-Huaral basin geometry as Phase-2 research context.

RESEARCH_ONLY / TEST_ONLY. The emitted polygon is only the official ANA
hydrologic unit 137558. It does not resolve tributary ravines, river reaches,
event footprints, exposure, hydraulic capacity, controls, thresholds or
activation.
"""
...
CANDIDATE_ID = "lima_norte_chancay_huaral"
QUERY_NAME = "Cuenca Chancay - Huaral"
EXPECTED_CODE = "137558"
SOURCE_ID = "ANA-IDEP-UH-CHANCAY-HUARAL-137558-20260923"
BASE = "https://www.idep.gob.pe/geoportal/rest/services/INSTITUCIONALES/ANA_WMS/MapServer/8/query"
SOURCE_DIR = ROOT / "site/data/phase2/sources/chancay_huaral_hydrologic_context"
SOURCE = SOURCE_DIR / "ana_cuenca_chancay_huaral_137558.geojson"
...
def validate_source(data: dict) -> dict:
    ...
    if data.get("type") != "FeatureCollection" or len(features) != 1:
        raise ValueError("expected exactly one official Chancay-Huaral feature")
    feature = features[0]
    ...
    if (feature.get("geometry") or {}).get("type") not in {"Polygon", "MultiPolygon"}:
        raise ValueError("official Chancay-Huaral geometry is not polygonal")
...
def source_record_from_inventory() -> dict:
    ...
    if len(sources) != 1 or sources[0].get("source_id") != SOURCE_ID:
        raise ValueError("unexpected Chancay-Huaral source inventory")
    return sources[0]
...
def expected_inventory_document(doc: dict) -> dict:
    ...
    if len(rows) != 1:
        raise ValueError("Chancay-Huaral candidate missing from Phase-2 inventory")
    row = rows[0]
...
def expected_contract_document(doc: dict, geometry_rel: str) -> dict:
    ...
    if updated.get("candidate_id") != CANDIDATE_ID:
        raise ValueError("Chancay-Huaral contract identity mismatch")
    if updated.get("deployment_status") != "RESEARCH_ONLY" or updated.get("production_use") is not False:
        raise ValueError("unsafe Chancay-Huaral contract state")
    if updated.get("decision_thresholds") is not None or updated.get("hydraulic_factors") is not None:
        raise ValueError("Chancay-Huaral decision fields unexpectedly populated")
    if updated.get("missing_data_rule") != "UNKNOWN_NOT_LOW_RISK":
        raise ValueError("Chancay-Huaral missing-data guard changed")
    if (updated.get("validation") or {}).get("activation_gate") != "BLOCKED":
        raise ValueError("Chancay-Huaral activation gate changed")
    ...
    note = (
        "Official ANA Cuenca Chancay - Huaral unit 137558 is normalized only as PARTIAL basin context; tributary ravines, river reaches, event footprints, exposure and hydraulic behavior remain unresolved and separate."
    )
    ...
    normalized = {
        ...
        "properties": {
            **GUARDS,
            ...
            "coverage": "Official ANA Cuenca Chancay - Huaral hydrologic-unit polygon only; tributary ravines and river reaches remain unresolved components.",
        ...
        "features": [{
            "type": "Feature",
            "id": "lima_norte_chancay_huaral_basin_context_137558",
            "properties": {
                "candidate_id": CANDIDATE_ID,
                "unit_id": "lima_norte_chancay_huaral_basin_context_137558",
                "name": "Cuenca Chancay–Huaral · contexto hidrológico ANA",
                ...
                "warning": "Basin context only; do not infer tributary routing, flood extent, hydraulic capacity, thresholds, risk or alert state.",
            },
            "geometry": feature["geometry"],
        }],
    }
    validation = {
        "version": "phase2-chancay-huaral-geometry-validation-v1",
        **GUARDS,
        ...
        "component_resolution": {
            "chancay_huaral_basin_polygon": "REPRODUCIBLE_OFFICIAL_GEOMETRY",
...
def sync(check_only: bool) -> None:
    ...
    if sha_bytes(canonical(source)) != record["canonical_sha256"]:
        raise ValueError("frozen ANA Chancay-Huaral canonical SHA-256 mismatch")
...
def refresh_source() -> None:
    ...
    props = feature["properties"]
    inventory = {
        "version": "phase2-chancay-huaral-source-inventory-v1",
        **GUARDS,
        "sources": [{
            "candidate_id": CANDIDATE_ID,
            "source_id": SOURCE_ID,
        ...
        "forbidden": [
            ...
            "invent tributary ravine or river-reach geometry",
...
def main() -> int:
    ...
    if args.refresh_source:
        ...
        if not SOURCE_INVENTORY.is_file():
            raise ValueError("frozen ANA Chancay-Huaral source inventory missing; use --refresh-source once")
