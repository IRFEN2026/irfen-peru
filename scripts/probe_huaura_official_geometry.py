#!/usr/bin/env python3
"""Freeze official ANA Huaura basin geometry as Phase-2 research context.

RESEARCH_ONLY / TEST_ONLY. The emitted polygon is only the official ANA
hydrologic unit 13756. It does not resolve Huacho/Sayan local ravines, river
reaches, event footprints, exposure, hydraulic capacity, controls, thresholds
or activation.
"""
...
CANDIDATE_ID = "lima_norte_huaura_huacho_sayan"
QUERY_NAME = "Cuenca Huaura"
EXPECTED_CODE = "13756"
SOURCE_ID = "ANA-IDEP-UH-HUAURA-13756-20260923"
BASE = "https://www.idep.gob.pe/geoportal/rest/services/INSTITUCIONALES/ANA_WMS/MapServer/8/query"
SOURCE_DIR = ROOT / "site/data/phase2/sources/huaura_hydrologic_context"
SOURCE = SOURCE_DIR / "ana_cuenca_huaura_13756.geojson"
SOURCE_INVENTORY = SOURCE_DIR / "source_inventory.json"
GEOMETRY = ROOT / "site/data/phase2/geometries/lima_norte_huaura_huacho_sayan_huaura_basin_context.geojson"
VALIDATION = ROOT / "site/data/phase2/geometries/lima_norte_huaura_huacho_sayan_geometry_validation.json"
CONTRACT = ROOT / "site/data/validation/phase2_zone_contracts/lima_norte_huaura_huacho_sayan.json"
...
def validate_source(data: dict) -> dict:
    ...
    if data.get("type") != "FeatureCollection" or len(features) != 1:
        raise ValueError("expected exactly one official Huaura feature")
    feature = features[0]
    ...
    if (feature.get("geometry") or {}).get("type") not in {"Polygon", "MultiPolygon"}:
        raise ValueError("official Huaura geometry is not polygonal")
...
def source_record_from_inventory() -> dict:
    ...
    if len(sources) != 1 or sources[0].get("source_id") != SOURCE_ID:
        raise ValueError("unexpected Huaura source inventory")
    return sources[0]
...
def expected_inventory_document(doc: dict) -> dict:
    ...
    if len(rows) != 1:
        raise ValueError("Huaura candidate missing from Phase-2 inventory")
    row = rows[0]
...
def expected_contract_document(doc: dict, geometry_rel: str) -> dict:
    ...
    note = (
        "Official ANA Cuenca Huaura unit 13756 is normalized only as PARTIAL basin context; Huacho/Sayan local ravines, individual river reaches, event footprints, exposure and hydraulic behavior remain unresolved and separate."
    )
    ...
    normalized = {
        ...
        "properties": {
            **GUARDS,
            ...
            "coverage": "Official ANA Cuenca Huaura hydrologic-unit polygon only; Huacho/Sayan local ravines and individual river reaches remain unresolved components.",
        ...
        "features": [{
            "type": "Feature",
            "id": "lima_norte_huaura_huacho_sayan_basin_context_13756",
            "properties": {
                "candidate_id": CANDIDATE_ID,
                "unit_id": "lima_norte_huaura_huacho_sayan_basin_context_13756",
                "name": "Cuenca Huaura · contexto hidrológico ANA",
                ...
                "outlet_used": False,
                "outlet": None,
                "local_ravines_geometry_resolved": False,
                ...
                "warning": "Basin context only; do not infer local-ravine routing, flood extent, hydraulic capacity, thresholds, risk or alert state.",
            },
            "geometry": feature["geometry"],
        }],
    }
    validation = {
        "version": "phase2-huaura-geometry-validation-v1",
        **GUARDS,
        ...
        "component_resolution": {
            "huaura_basin_polygon": "REPRODUCIBLE_OFFICIAL_GEOMETRY",
            "huacho_sayan_local_ravines": "UNRESOLVED_NO_GEOMETRY_DRAWN",
            "individual_river_reaches": "UNRESOLVED_NO_GEOMETRY_DRAWN",
...
def sync(check_only: bool) -> None:
    ...
    if sha_bytes(canonical(source)) != record["canonical_sha256"]:
        raise ValueError("frozen ANA Huaura canonical SHA-256 mismatch")
...
def refresh_source() -> None:
    ...
    props = feature["properties"]
    inventory = {
        "version": "phase2-huaura-source-inventory-v1",
        **GUARDS,
        "sources": [{
            "candidate_id": CANDIDATE_ID,
            "source_id": SOURCE_ID,
        ...
        "forbidden": [
            "treat basin polygon as an event or inundation footprint",
            "invent Huacho/Sayan local-ravine or individual river-reach geometry",
...
def main() -> int:
    ...
    if args.refresh_source:
        ...
        if not SOURCE_INVENTORY.is_file():
            raise ValueError("frozen ANA Huaura source inventory missing; use --refresh-source once")
