#!/usr/bin/env python3
"""Resolve Carossio terrain-derived pour point from pre-unblind static geometry only."""
...
CONTRACT=ROOT/'config/chosica_2015_carossio_outlet_resolution_contract_v0_1.json'
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='carossio_') as raw:
  ...
  path=set(main[:i+1]);starts=cells(t,w,h,*r6);rep['carossio_source_cells']=[{'row':r,'col':c} for r,c in starts];ints=[];di=[]
  for st in starts:
   tr=trace(fd,st);first=next((z for z in tr if z in path),None);di.append({'carossio_start':list(st),'status':'FIRST_INTERSECTION_FOUND' if first else 'NO_INTERSECTION',**({'row':first[0],'col':first[1]} if first else {})})
  ...
  if len(ints)!=len(starts):rep['status']='PENDING_REVIEW_MISSING_CAROSSIO_RIMAC_INTERSECTION'
  elif len(uq)!=1:rep['status']='PENDING_REVIEW_NONCONVERGENT_CAROSSIO_OUTLET'
  else:
   r,c=uq[0];x,y=center(t,r,c);lon,lat=tg.transform(x,y);rep['accepted_outlet']={'row':r,'col':c,'x_m':round(x,3),'y_m':round(y,3),'lon':round(lon,8),'lat':round(lat,8)};rep['status']='PASS_CAROSSIO_D8_HYDROLOGIC_POUR_POINT_CANDIDATE';rep['freeze_eligible']=True
