...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--manifest',required=True,type=Path); ap.add_argument('--features',required=True,type=Path); ap.add_argument('--timebasis',required=True,type=Path); ap.add_argument('--sampling-contract',required=True,type=Path); ap.add_argument('--sentinel-r1',required=True,type=Path); ap.add_argument('--sentinel-r2-contract',required=True,type=Path); a=ap.parse_args()
    m=json.loads(a.manifest.read_text(encoding='utf-8')); f=json.loads(a.features.read_text(encoding='utf-8')); t=json.loads(a.timebasis.read_text(encoding='utf-8')); s=json.loads(a.sampling_contract.read_text(encoding='utf-8')); r=json.loads(a.sentinel_r1.read_text(encoding='utf-8')); r2=json.loads(a.sentinel_r2_contract.read_text(encoding='utf-8'))
    ...
    assert r['territorial_activation_evidence_blinded'] is True and r2['territorial_activation_evidence_blinded'] is True
    local=t['peru_local_calendar_features']; ant=local['antecedent_ending_local_00']
    m['version']='irfen-independent-basin-validation-map-v1.6'
    m['generated_at']=now()
    ...
    c['framework_stage']='A4_R1_NATIVE_RADIOMETRIC_COMPLETE_R2_PREREGISTERED_BLOCKED_AUX_IDENTITY'
    ...
    c['remote_sensing_status']='S1_A4_R1_COMPLETE_R2_PREREGISTERED_BLOCKED_VERTICAL_DATUM_ORBIT_AUX_LANDSAT_QA_DIAGNOSTIC_ONLY'
    ...
    print(json.dumps({'version':m['version'],'stage':c['framework_stage'],'sentinel1_r1_status':c['sentinel1_r1_status'],'sentinel1_r2_status':c['sentinel1_r2_status'],'gate':m['serious_modeling_gate']},indent=2,ensure_ascii=False)); return 0
