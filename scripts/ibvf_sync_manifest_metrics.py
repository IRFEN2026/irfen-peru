...
def main():
    ...
    assert pool['summary']['unique_calendar_days_per_track']==2907 and pool['summary']['track_day_windows']==8721
    assert pool['summary']['days_removed_for_sensor_missing']==0 and pool['summary']['days_removed_for_outcome']==0
    objs=(m,f,t,s,r,r2,rp,pool)
    ...
    assert all(x.get('territorial_activation_evidence_blinded') is True for x in (m,r,r2,rp,pool))
    local=t['peru_local_calendar_features']; ant=local['antecedent_ending_local_00']
    m['version']='irfen-independent-basin-validation-map-v1.7'
    m['generated_at']=now()
    ...
    m['parallel_a0_pool_status']='FROZEN_EXHAUSTIVE_CALENDAR_NO_WINDOW_SELECTED'
    m['parallel_a0_pool_summary']={
      'tracks':pool['tracks'],
      'season_count':pool['summary']['season_count'],
      'unique_calendar_days_per_track':pool['summary']['unique_calendar_days_per_track'],
      'track_day_windows':pool['summary']['track_day_windows'],
      'case_control_assignment_allowed':False,
      'meteorological_ranking_status':pool['meteorological_ranking_status']
    }
    ...
    ac['parallel_a0_pool']='site/data/validation/ibvf_parallel_a0_pool_inventory.json'
    ...
    ac['sentinel1_r2_contract']='site/data/validation/cashahuacra_sentinel1_r2_contract.json'
    ac['sentinel1_r2_prerequisites_contract']='site/data/validation/cashahuacra_sentinel1_r2_prerequisites_contract.json'
    ac['sentinel1_r2_prerequisites_script']='scripts/ibvf_sentinel1_r2_freeze_prerequisites.py'
    ...
    lv['parallel_a0_pool']='FROZEN_2907_DAYS_PER_LIMA_TRACK_8721_TRACK_DAY_WINDOWS_NO_SELECTION'
    lv['github_actions_sentinel1_a4_r1']='SUCCESS_BOTH_DATES_NATIVE_RADIOMETRIC_NO_COMPARISON'
    lv['sentinel1_a4_r2_contract']='PREREGISTERED_BLOCKED_VERTICAL_DATUM_AND_ORBIT_AUX_IDENTITY'
    lv['sentinel1_a4_r2_prerequisite_freezer']='IMPLEMENTED_EXECUTION_PENDING'
    c=next(x for x in m['cases'] if x.get('unit_id')=='cashahuacra')
    c['framework_stage']='A4_R1_COMPLETE_R2_PREREQUISITE_FREEZER_IMPLEMENTED_GATE_PENDING'
    ...
    c['remote_sensing_status']='S1_A4_R1_COMPLETE_R2_PREREQUISITE_RESOURCE_FREEZE_PENDING_LANDSAT_QA_DIAGNOSTIC_ONLY'
    ...
    c['sentinel1_r2_contract_path']='data/validation/cashahuacra_sentinel1_r2_contract.json'
    c['sentinel1_r2_prerequisites_contract_path']='data/validation/cashahuacra_sentinel1_r2_prerequisites_contract.json'
    c['sentinel1_r2_prerequisites_status']=rp['status']
    c['sentinel1_r2_status']=r2['status']
    c['sentinel1_r2_vertical_datum_gate']=r2['vertical_datum_gate']['status']
    c['sentinel1_r2_orbit_gate']=r2['orbit_gate']['status']
    for unit in pool['tracks']:
        pc=next(x for x in m['cases'] if x.get('unit_id')==unit)
        pc['framework_stage']='A0_EXHAUSTIVE_POOL_FROZEN_NO_WINDOW_SELECTED'
        pc['blind_window']='NOT_YET_SELECTED_FROM_FROZEN_POOL'
        pc['a0_pool_status']='FROZEN_2907_UNASSIGNED_LOCAL_DAYS'
        pc['a0_pool_path']='data/validation/ibvf_parallel_a0_pool_inventory.json'
        pc['a0_case_control_role']='UNASSIGNED'
        pc['blind_status']='TERRITORIAL_EVIDENCE_SEALED'
    a.manifest.write_text(json.dumps(m,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    print(json.dumps({'version':m['version'],'cashahuacra_stage':c['framework_stage'],'r2_prerequisite':c['sentinel1_r2_prerequisites_status'],'parallel_pool':m['parallel_a0_pool_status'],'track_day_windows':pool['summary']['track_day_windows'],'gate':m['serious_modeling_gate']},indent=2,ensure_ascii=False)); return 0
