#!/usr/bin/env python3
"""Bounded official IGN hydrography probe for the Jicamarca Rio Seco child.

This is a source-discovery gate only. It queries official IDEP/IGN base
hydrography inside the preregistered documentary Jicamarca window and returns
candidate polylines for a later independent identity/topology review. It never
accepts a Rio Seco channel, catchment, outlet, confluence, routing parameter,
event footprint, negative control, threshold, risk state or alert.
"""
...
DEFAULT = ROOT / "config/phase2_jicamarca_rio_seco_ign_hydrography_probe_v0_1.json"
SAFE = {
...
def candidate_row(feature: dict, output_wkid: int) -> dict:
    ...
    return {
        ...
        "basin_text": normalize_text(attrs.get("CUENCA")),
        "department": normalize_text(attrs.get("DPTO")),
        "province": normalize_text(attrs.get("PROVINCIA")),
        "district": normalize_text(attrs.get("DISTRITO")),
        "cod_facc": normalize_text(attrs.get("COD_FACC")),
        "cod_ign": normalize_text(attrs.get("COD_IGN")),
        "source_geometry": {
...
def main() -> None:
    ...
    out_fields = ",".join(source["required_fields"] + ["OBJECTID"])
    ...
    candidates.sort(
        key=lambda row: (
            ...
            str(row.get("province") or "").casefold(),
            str(row.get("district") or "").casefold(),
    ...
    report = {
        "schema_version": "0.1",
        "status": "PASS_BOUNDED_IGN_RIO_SECO_HYDROGRAPHY_CANDIDATE_PROBE",
        **SAFE,
        ...
        "interpretation_rule": "Every returned IGN line is candidate-only. Name and intersection with the coarse documentary Jicamarca window are insufficient identity. Promotion requires a separate frozen, outcome-independent identity/topology review compatible with independent official evidence and must keep Rio Seco distinct from Colca and Huaycoloro.",
    }
    guard(report, "REPORT")
