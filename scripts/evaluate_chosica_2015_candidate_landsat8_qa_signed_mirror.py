#!/usr/bin/env python3
"""Run frozen Landsat QA through an ephemeral signed transport mirror.
Scientific semantics remain those of the frozen QA contract. Signed URLs are used only
in memory and are never persisted or logged. Full files must match canonical USGS
SHA-512 before rasterio is allowed to open them.
"""
...
def signed_url(unsigned: str, allowed_host: str) -> str:
    uu=urllib.parse.urlparse(unsigned)
    if uu.scheme!='https' or uu.hostname!=allowed_host or uu.query:
        raise RuntimeError('FAIL_CLOSED_UNSIGNED_MIRROR_ASSET_SCOPE')
    signed=planetary_computer.sign_url(unsigned)
    su=urllib.parse.urlparse(signed)
    if su.scheme!='https' or su.hostname!=uu.hostname or su.path!=uu.path or not su.query:
        raise RuntimeError('FAIL_CLOSED_SIGNED_URL_INVARIANTS')
    return signed

def download_exact_signed(session,unsigned,checksum,path,allowed_host):
    signed=signed_url(unsigned,allowed_host)
    h=hashlib.sha512();n=0
    with session.get(signed,stream=True,timeout=(20,300),allow_redirects=False) as r:
        if r.is_redirect or r.is_permanent_redirect: raise RuntimeError('FAIL_CLOSED_SIGNED_ASSET_REDIRECT')
        if r.status_code!=200: raise RuntimeError(f'FAIL_CLOSED_SIGNED_ASSET_HTTP_{r.status_code}')
        with path.open('wb') as f:
            for c in r.iter_content(1<<20):
                if c: f.write(c);h.update(c);n+=len(c)
    got=h.hexdigest();exp=expected_sha512(checksum)
    if got!=exp: raise RuntimeError(f'FAIL_CLOSED_SIGNED_ASSET_SHA512_MISMATCH expected={exp} got={got}')
    return got,n

def fail_report(base,output,error,transfer_started):
    d=dict(base);d.update({'status':'FAIL_CLOSED_LANDSAT8_QA_SIGNED_MIRROR_TRANSPORT','error':str(error),'qa_pixel_asset_transfer_started':bool(transfer_started),'qa_pixel_asset_checksum_verified_all':False,'qa_pixel_raster_read':False,'signed_url_persisted':False,'signed_url_logged':False,'reflectance_asset_bytes_read':False,'outcome_evidence_read':False,'target_names_or_ids_read':False,'a6680_read':False,'contaminated_adjudication_used':False,'scene_selection_modified':False,'candidate_selection_modified':False,'candidate_ranking_modified':False,'free_web_search_used':False,'control_labels_assigned':False,'sealed_target_unblind_allowed':False})
    output.parent.mkdir(parents=True,exist_ok=True);output.write_text(json.dumps(d,sort_keys=True,separators=(',',':'))+'\n',encoding='utf-8')

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--overlay',type=Path,required=True);ap.add_argument('--scientific-contract',type=Path,required=True);ap.add_argument('--inventory',type=Path,required=True);ap.add_argument('--mirror',type=Path,required=True);ap.add_argument('--geometry',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args()
    ov=json.loads(a.overlay.read_text());co=json.loads(a.scientific_contract.read_text());inv=json.loads(a.inventory.read_text());mir=json.loads(a.mirror.read_text());gj=json.loads(a.geometry.read_text())
    base={'schema_version':'0.3','batch_id':co['batch_id'],'guards':co['guards'],'transport_overlay_sha256':sha256(a.overlay),'scientific_contract_git_blob_sha':git_blob_sha(a.scientific_contract),'asset_inventory_sha256':sha256(a.inventory),'transport_mirror_sha256':sha256(a.mirror),'candidate_geometry_sha256':sha256(a.geometry),'transport_mirror_role':'TRANSPORT_ONLY_NOT_SCIENTIFIC_SOURCE','signing_library':'planetary-computer','signing_library_version':pkg_version('planetary-computer')}
    try:
        ...
        if mir.get('status')!='PASS_LANDSAT8_TRANSPORT_MIRROR_EXACT_ITEM_IMMUTABLE_METADATA_ONLY' or mir.get('mirror_role')!='TRANSPORT_ONLY_NOT_SCIENTIFIC_SOURCE': raise RuntimeError('FAIL_CLOSED_MIRROR_STATUS')
    ...
    session=requests.Session();session.headers.update({'User-Agent':'IRFEN-research-cleanroom/qa-signed-mirror-0.1'})
    invalid_bits=[int(x) for x in co['qa_pixel']['invalid_bits'].keys()];invalid_mask=sum(1<<b for b in invalid_bits);provenance={};arrays={};transfer_started=False
    with tempfile.TemporaryDirectory(prefix='irfen_l8qa_signed_') as raw:
        ...
        try:
            for role in roles:
                ...
                ca=[x for x in item['assets'] if x['asset_key']==co['asset_policy']['allowed_asset_key']];ma=[x for x in mi['assets'] if x['asset_key']==co['asset_policy']['allowed_asset_key']]
                ...
                p=td/f'{role}_qa.tif';transfer_started=True;digest,n=download_exact_signed(session,mirror['href'],canonical.get('file:checksum'),p,ov['transport_mirror']['unsigned_asset_host']);verified.add(str(p))
                provenance[role]={'item_id':iid,'mirror_item_id':mi['mirror_item_id'],'asset_key':'qa_pixel','sha512':digest,'bytes':n,'canonical_usgs_multihash':canonical.get('file:checksum'),'transport_host':urllib.parse.urlparse(mirror['href']).hostname,'signed_transport_used':True,'signed_url_persisted':False,'signed_url_logged':False,'checksum_verified_before_raster_read':True}
            for role in roles:
                p=td/f'{role}_qa.tif'
        ...
        for role in ('reference','post'):
            src,sm=arrays[role];dst=np.full(target_shape,1,dtype=src.dtype);reproject(source=src,destination=dst,src_transform=sm['transform'],src_crs=sm['crs'],src_nodata=None,dst_transform=pm['transform'],dst_crs=pm['crs'],dst_nodata=1,resampling=Resampling.nearest);aligned[role]=dst
        valid={r:(np.bitwise_and(aligned[r].astype('uint32'),invalid_mask)==0) for r in roles};feats=gj.get('features',[]);outrows=[];gate=co['quality_gate']
        for f in sorted(feats,key=lambda z:str((z.get('properties') or {}).get('candidate_code',''))):
            code=(f.get('properties') or {}).get('candidate_code');geom=transform_geom('EPSG:4326',pm['crs'],f['geometry'],precision=3);cm=geometry_mask([geom],out_shape=target_shape,transform=pm['transform'],all_touched=False,invert=True);total=int(cm.sum())
    ...
    usable=sum(x['qa_status']==co['quality_gate']['candidate_status_if_pass'] for x in outrows);out=dict(base);out.update({'status':'PASS_LANDSAT8_CANDIDATE_QA_GATE' if usable>=int(co['quality_gate']['minimum_usable_candidates_to_continue']) else 'FAIL_CLOSED_LANDSAT8_QA_NO_USABLE_CANDIDATES','invalid_qa_bits':invalid_bits,'alignment':co['alignment'],'asset_provenance':provenance,'candidate_count':len(outrows),'usable_candidate_count':usable,'candidates':outrows,'qa_pixel_asset_transfer_started':True,'qa_pixel_asset_checksum_verified_all':True,'qa_pixel_raster_read':True,'signed_transport_used':True,'signed_url_persisted':False,'signed_url_logged':False,'reflectance_asset_bytes_read':False,'outcome_evidence_read':False,'target_names_or_ids_read':False,'a6680_read':False,'contaminated_adjudication_used':False,'scene_selection_modified':False,'candidate_selection_modified':False,'candidate_ranking_modified':False,'free_web_search_used':False,'control_labels_assigned':False,'sealed_target_unblind_allowed':False})
    a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(out,sort_keys=True,separators=(',',':'))+'\n',encoding='utf-8');print(json.dumps({'status':out['status'],'candidate_count':len(outrows),'usable_candidate_count':usable,'all_qa_sha512_exact':True,'signing_library_version':out['signing_library_version']},sort_keys=True))
