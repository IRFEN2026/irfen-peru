"""Regression tests for official ANA Cuenca Omas Phase-2 research geometry."""
...
SCRIPT = ROOT / "scripts/probe_asia_omas_official_geometry.py"
SOURCE_INVENTORY = ROOT / "site/data/phase2/sources/asia_omas_hydrologic_context/source_inventory.json"
...
class AsiaOmasGeometryTests(unittest.TestCase):
    ...
    def test_frozen_official_source_identity_and_hash(self):
        ...
        row = inv["sources"][0]
        self.assertEqual(row["official_unit_code"], "1375512")
        self.assertEqual(row["official_unit_name"], "Cuenca Omas")
        self.assertAlmostEqual(float(row["official_area_km2"]), 1111.12, delta=1.0)
    ...
    def test_geometry_is_only_official_omas_basin_context(self):
        doc = load(GEOMETRY)
    ...
    def test_map_is_research_context_only(self):
        ...
        row = catalog["lima_sur_asia_omas"]
        ...
        mapped = {row["candidate_id"]: row for row in load(MAP)["research_zones"]}["lima_sur_asia_omas"]
