...
def test_huamanazana_exact_identity_and_chorobal_remains_separate():
 c=load(C);p=load(P);kids={x["child_id"]:x for x in p["hydrologic_children"]};h=kids[CID];ch=kids["chorobal_river_component"];ci={x["child_id"]:x for x in c["hydrologic_identity"]["children"]}[CID]
 assert h["ana_unit_code"]=="137712" and h["name"]=="Cuenca Huamanzaña";assert ci["ana_unit_name"]=="Cuenca Huamansaña";assert ci["display_name"]=="Cuenca Huamanzaña";assert h["geometry"]["source_query"]["where"]=="CODIGO='137712'"
 if c["assets"]["geometry_components"]:assert h["ana_unit_name"]=="Cuenca Huamansaña" and "Cuenca Huamanzaña" in h["name_aliases"]
...
def test_frozen_geometry_hashes_and_scientific_guards():
 ...
 fp=geo["features"][0]["properties"];assert fp["official_unit_code"]=="137712";assert fp["official_name"]=="Cuenca Huamansaña";assert fp["name"]=="Cuenca Huamanzaña";assert fp["counts_as_event_footprint"] is False;assert fp["counts_as_operational_geometry"] is False;assert fp["alerting_enabled"] is False
 v=load(V);assert v["ana_unit_name"]=="Cuenca Huamansaña";assert v["display_name"]=="Cuenca Huamanzaña";assert v["outcomes_read"] is False;assert v["event_footprint_created"] is False;assert v["hydraulic_capacity_read"] is False;assert v["thresholds_used"] is False;assert v["negative_controls_read"] is False;assert v["chorobal_geometry_inferred"] is False;assert v["parent_composite_created"] is False
