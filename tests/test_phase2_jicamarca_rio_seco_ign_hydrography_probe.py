...
CONFIG = ROOT / "config/phase2_jicamarca_rio_seco_ign_hydrography_probe_v0_1.json"
...
def test_query_uses_independent_official_hydrography_and_coarse_window_only():
    doc = load()
    source = doc["source"]
    assert source["institution"] == "Instituto Geografico Nacional del Peru"
    assert source["publication_host"] == "Infraestructura de Datos Espaciales del Peru (IDEP)"
    assert source["layer_id"] == 5
    assert source["expected_layer_name"] == "Corriente de Agua"
    assert source["expected_geometry_type"] == "esriGeometryPolyline"
    assert source["source_role"].startswith("OFFICIAL_BASE_HYDROGRAPHY_CANDIDATE_SOURCE")
