...
SOURCE_INVENTORY = ROOT / "site/data/phase2/sources/huaura_hydrologic_context/source_inventory.json"
MAP_CATALOG = ROOT / "site/data/map_layers.json"


class HuauraGeometryTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_frozen_source_replays_exactly(self):
        subprocess.run(
            [sys.executable, "scripts/probe_huaura_official_geometry.py", "--check-only"],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
    ...
    def test_geometry_is_official_partial_basin_context_only(self):
        ...
        self.assertEqual(v["official_unit"]["code"], "13756")
        self.assertEqual(v["official_unit"]["name"], "Cuenca Huaura")
        self.assertFalse(v["counts_as_complete_candidate_geometry"])
        self.assertFalse(v["artificial_connector_used"])
        self.assertEqual(v["component_resolution"]["huacho_sayan_local_ravines"], "UNRESOLVED_NO_GEOMETRY_DRAWN")
        self.assertEqual(v["component_resolution"]["individual_river_reaches"], "UNRESOLVED_NO_GEOMETRY_DRAWN")
        ...
        self.assertEqual(feature["properties"]["official_hydrologic_unit_code"], "13756")
        ...
        self.assertFalse(feature["properties"]["local_ravines_geometry_resolved"])
        self.assertFalse(feature["properties"]["river_reaches_geometry_resolved"])
    ...
    def test_source_identity_and_map_catalog_are_linked(self):
        ...
        self.assertEqual(source["official_unit_code"], "13756")
        self.assertEqual(source["official_unit_name"], "Cuenca Huaura")
        ...
        rows = [r for r in catalog["research_zones"] if r["candidate_id"] == "lima_norte_huaura_huacho_sayan"]
        self.assertEqual(len(rows), 1)
        self.assertTrue(rows[0]["geometry"]["map_eligible"])
        self.assertFalse(rows[0]["geometry"]["default_visibility"])
        ...
        self.assertEqual(rows[0]["validation"]["activation_gate"], "BLOCKED")
