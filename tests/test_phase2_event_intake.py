...
class Phase2EventIntakeTests(unittest.TestCase):
    ...
    def test_official_outcome_with_reproducible_geometry_is_ready_for_reanalysis(self):
    ...
    def test_nonofficial_geometry_requires_research_provenance(self):
        path = ROOT / "site/data/validation/phase2_event_intake/villa-el-salvador-2026-08-16-coen.json"
        row = json.loads(path.read_text(encoding="utf-8"))
        row["reported_location"]["coordinates"].pop("source_url")
        with self.assertRaises(phase2_events.EventIntakeError):
            phase2_events.validate_event(row)
    ...
    def test_public_catalog_is_fail_closed_and_committed(self):
        ...
        self.assertEqual(generated["summary"]["verified_pending_geometry"], 0)
        self.assertEqual(generated["summary"]["ready_for_reanalysis"], 1)
