(() => {
  ...
  async function render(){
    ...
    const d=await safeJson('data/phase2/catalog.json');
    if(!d||d.production_use!==false){body.innerHTML='<div class="histnote">El catálogo de expansión todavía no ha sido generado.</div>';return;}
    const s=d.summary||{};
    body.innerHTML=`<div class="card" style="margin:0 0 14px;border:2px solid #e4c06b;background:#fffaf0">
      ...
      <div class="small" style="margin-top:8px"><b>Contratos:</b> ${esc(s.contracts_present)} · <b>fuera de Lima Metropolitana:</b> ${esc(s.outside_lima_metropolitana)} · <b>zonas operativas nuevas:</b> 0.</div>
    </div>
    ...
    <div class="cards" style="grid-template-columns:repeat(auto-fit,minmax(310px,1fr))">${(d.zones||[]).map(zoneCard).join('')}</div>`;
  }
